...
class OrganizationModel extends Model
{
	...
	public function createOrganization( $owner_id=null, $name=null, $desc=null )
	{
		...
		if( is_array( $data ) && count( $data ) > 0 ) {
			...
			if( $newOrg->rowCount() ) {
				...
				$this->createOrganizationMember($this->_db->lastInsertId(), $owner_id, self::ROLE_ADMIN, self::STATUS_ACTIVE );
	...
	public function getOrganizationsMembers($org_id)
	{
		...
		$sql = "SELECT CONCAT(u.first_name,' ',u.last_name) AS full_name,u.email,o.id,o.name,om.role_id,om.date_joined,om.status ";
		$sql .= "FROM organization_members AS om ";
		$sql .= "INNER JOIN organization AS o ";
		$sql .= "ON om.organization_id=o.id ";
